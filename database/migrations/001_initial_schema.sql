...
plan_type VARCHAR(20) DEFAULT 'free' CHECK (
    plan_type IN (
        'free',
        'premium',
        'enterprise'
...
country VARCHAR(2), city VARCHAR(100) );
...
-- Page Info
page_path VARCHAR(500), page_title VARCHAR(255),
...
-- Vendors policies
CREATE POLICY vendor_isolation ON vendors FOR ALL USING (auth.uid () = user_id);

CREATE POLICY vendor_public_read ON vendors FOR
SELECT USING (status = 'active');

-- Categories policies
CREATE POLICY category_vendor_access ON categories FOR ALL USING (
    vendor_id IN (
        SELECT id
        FROM vendors
        WHERE
...
CREATE POLICY category_public_read ON categories FOR
SELECT USING (
        is_active = true
        AND vendor_id IN (
            SELECT id
...
-- Products policies
CREATE POLICY product_vendor_access ON products FOR ALL USING (
    vendor_id IN (
        SELECT id
        FROM vendors
        WHERE
...
CREATE POLICY product_public_read ON products FOR
SELECT USING (
        status = 'active'
        AND vendor_id IN (
            SELECT id
...
CREATE POLICY product_images_vendor_access ON product_images FOR ALL USING (
    product_id IN (
        SELECT id
        FROM products
        WHERE
...
CREATE POLICY product_images_public_read ON product_images FOR
SELECT USING (
        product_id IN (
            SELECT id
            FROM products
...
CREATE POLICY qr_scans_vendor_access ON qr_scans FOR ALL USING (
    vendor_id IN (
        SELECT id
        FROM vendors
        WHERE
...
CREATE POLICY contact_interactions_vendor_access ON contact_interactions FOR ALL USING (
    vendor_id IN (
        SELECT id
        FROM vendors
        WHERE
...
CREATE POLICY page_views_vendor_access ON page_views FOR ALL USING (
    vendor_id IN (
        SELECT id
        FROM vendors
        WHERE
...
CREATE POLICY qr_scans_anonymous_insert ON qr_scans FOR INSERT
WITH
    CHECK (true);

CREATE POLICY contact_interactions_anonymous_insert ON contact_interactions FOR INSERT
WITH
    CHECK (true);

CREATE POLICY page_views_anonymous_insert ON page_views FOR INSERT
WITH
    CHECK (true);
...
FROM
    ...
    AND p.status = 'active'
    LEFT JOIN categories c ON v.id = c.vendor_id
...
-- The vendor_stats view will automatically respect RLS policies
-- from the vendors, products, and categories tables